...
// Routes are project-scoped: /{projectId}/graphql. MySQL e2e runs single-database
// in insecure dev mode, pinned to 'e2e-test' (APP_PROJECT_ID); any other project
...
describe('Basic queries', () => {
  ...
  test('filter by neq', async () => {
    const data = await client.request(gql`{ hanaCustomer(where: { first_name: { neq: "MARY" } }) { customer_id first_name } }`);
    data.hanaCustomer.forEach(r => expect(r.first_name).not.toBe('MARY'));
  });

  test('filter by gt (total > 50)', async () => {
    const data = await client.request(gql`{ hanaOrders(where: { total: { gt: 50 } }) { order_id total } }`);
    expect(data.hanaOrders.length).toBeGreaterThanOrEqual(1);
    data.hanaOrders.forEach(r => expect(r.total).toBeGreaterThan(50));
  });

  test('filter by gte (total >= 99.99)', async () => {
    const data = await client.request(gql`{ hanaOrders(where: { total: { gte: 99.99 } }) { order_id total } }`);
    expect(data.hanaOrders.length).toBeGreaterThanOrEqual(1);
    data.hanaOrders.forEach(r => expect(r.total).toBeGreaterThanOrEqual(99.99));
  });

  test('filter by lt (total < 20)', async () => {
    const data = await client.request(gql`{ hanaOrders(where: { total: { lt: 20 } }) { order_id total } }`);
    expect(data.hanaOrders.length).toBeGreaterThanOrEqual(1);
    data.hanaOrders.forEach(r => expect(r.total).toBeLessThan(20));
  });

  test('filter by lte (total <= 9.99)', async () => {
    const data = await client.request(gql`{ hanaOrders(where: { total: { lte: 9.99 } }) { order_id total } }`);
    expect(data.hanaOrders.length).toBeGreaterThanOrEqual(1);
    data.hanaOrders.forEach(r => expect(r.total).toBeLessThanOrEqual(9.99));
  });

  test('filter by contains', async () => {
    const data = await client.request(gql`{ hanaCustomer(where: { first_name: { contains: "AR" } }) { customer_id first_name } }`);
    expect(data.hanaCustomer.length).toBeGreaterThanOrEqual(1);
  });

  test('filter by startsWith', async () => {
    const data = await client.request(gql`{ hanaCustomer(where: { first_name: { startsWith: "MA" } }) { customer_id first_name } }`);
    expect(data.hanaCustomer.length).toBeGreaterThanOrEqual(1);
    data.hanaCustomer.forEach(r => expect(r.first_name.startsWith('MA')).toBe(true));
  });

  test('filter by endsWith', async () => {
    const data = await client.request(gql`{ hanaCustomer(where: { last_name: { endsWith: "SON" } }) { customer_id last_name } }`);
    expect(data.hanaCustomer.length).toBeGreaterThanOrEqual(1);
    data.hanaCustomer.forEach(r => expect(r.last_name.endsWith('SON')).toBe(true));
  });

  test('filter by like', async () => {
    const data = await client.request(gql`{ hanaCustomer(where: { email: { like: "%@example.com" } }) { customer_id email } }`);
    expect(data.hanaCustomer.length).toBeGreaterThanOrEqual(5);
  });

  test('filter by isNull', async () => {
    const data = await client.request(gql`{ hanaCustomer(where: { email: { isNull: true } }) { customer_id email } }`);
    data.hanaCustomer.forEach(r => expect(r.email).toBeNull());
  });

  test('filter by isNotNull', async () => {
    const data = await client.request(gql`{ hanaCustomer(where: { email: { isNotNull: true } }) { customer_id email } }`);
    expect(data.hanaCustomer.length).toBeGreaterThanOrEqual(10);
    data.hanaCustomer.forEach(r => expect(r.email).not.toBeNull());
  });

  test('filter by in list', async () => {
    const data = await client.request(gql`{ hanaOrders(where: { status: { in: ["delivered", "shipped"] } }) { order_id status } }`);
    expect(data.hanaOrders.length).toBeGreaterThanOrEqual(1);
    data.hanaOrders.forEach(r => expect(['delivered', 'shipped']).toContain(r.status));
  });

  test('filter by notIn list', async () => {
    const data = await client.request(gql`{ hanaOrders(where: { status: { notIn: ["cancelled"] } }) { order_id status } }`);
    expect(data.hanaOrders.length).toBeGreaterThanOrEqual(1);
    data.hanaOrders.forEach(r => expect(r.status).not.toBe('cancelled'));
  });

  test('pagination limit', async () => {
    const data = await client.request(gql`{ hanaCustomer(limit: 5) { customer_id } }`);
    expect(data.hanaCustomer.length).toBe(5);
  });

  test('pagination limit + offset', async () => {
    const data = await client.request(gql`{ hanaCustomer(limit: 3, offset: 5) { customer_id } }`);
    expect(data.hanaCustomer.length).toBe(3);
  });

  test('ordering ASC', async () => {
    const data = await client.request(gql`{ hanaCustomer(orderBy: { customer_id: "ASC" }, limit: 3) { customer_id } }`);
    expect(data.hanaCustomer[0].customer_id).toBeLessThan(data.hanaCustomer[1].customer_id);
  });

  test('ordering DESC', async () => {
    const data = await client.request(gql`{ hanaCustomer(orderBy: { customer_id: "DESC" }, limit: 3) { customer_id } }`);
    expect(data.hanaCustomer[0].customer_id).toBeGreaterThan(data.hanaCustomer[1].customer_id);
  });

  test('orders basic query', async () => {
    const data = await client.request(gql`{ hanaOrders { order_id customer_id total status } }`);
    expect(data.hanaOrders.length).toBeGreaterThanOrEqual(10);
  });

  test('orders filter by status', async () => {
    const data = await client.request(gql`{ hanaOrders(where: { status: { eq: "delivered" } }) { order_id status } }`);
    expect(data.hanaOrders.length).toBeGreaterThanOrEqual(1);
  });

  test('products basic query', async () => {
    const data = await client.request(gql`{ hanaProduct { product_id name price stock } }`);
    expect(data.hanaProduct.length).toBeGreaterThanOrEqual(5);
  });

  test('filter by or — union of conditions', async () => {
    const data = await client.request(gql`{
      hanaCustomer(where: { or: [{ first_name: { eq: "MARY" } }, { first_name: { eq: "PATRICIA" } }] }) {
        customer_id first_name
      }
    }`);
    expect(data.hanaCustomer.length).toBeGreaterThanOrEqual(2);
    data.hanaCustomer.forEach(r => expect(['MARY', 'PATRICIA']).toContain(r.first_name));
  });

  test('active column is Int (0/1), not Boolean (tinyInt1isBit=false)', async () => {
    const data = await client.request(gql`{ hanaCustomer(limit: 3) { customer_id active } }`);
    data.hanaCustomer.forEach(r => {
...
describe('Aggregate queries', () => {
  ...
  test('orders aggregate count', async () => {
    ...
    expect(data.hanaOrdersAggregate.count).toBeGreaterThanOrEqual(10);
  });

  test('orders aggregate sum', async () => {
    const data = await client.request(gql`{ hanaOrdersAggregate { sum { total } } }`);
    expect(data.hanaOrdersAggregate.sum.total).toBeGreaterThan(0);
  });

  test('orders aggregate avg', async () => {
    const data = await client.request(gql`{ hanaOrdersAggregate { avg { total } } }`);
    expect(data.hanaOrdersAggregate.avg.total).toBeGreaterThan(0);
  });

  test('orders aggregate min <= max', async () => {
    const data = await client.request(gql`{ hanaOrdersAggregate { min { total } max { total } } }`);
    expect(data.hanaOrdersAggregate.min.total).toBeLessThanOrEqual(data.hanaOrdersAggregate.max.total);
  });
});
...
describe('Connection (cursor pagination)', () => {
  test('customerConnection returns edges + pageInfo', async () => {
    const data = await client.request(gql`{
      hanaCustomerConnection(first: 5) {
        edges { node { customer_id first_name } cursor }
        pageInfo { hasNextPage hasPreviousPage }
      }
    }`);
    expect(data.hanaCustomerConnection.edges.length).toBe(5);
  });
  ...
  test('customerConnection totalCount >= 10', async () => {
    const data = await client.request(gql`{ hanaCustomerConnection(first: 5) { totalCount } }`);
    expect(data.hanaCustomerConnection.totalCount).toBeGreaterThanOrEqual(10);
  });

  test('customerConnection with where filter returns only matching rows', async () => {
    const data = await client.request(gql`{
      hanaCustomerConnection(first: 100, where: { first_name: { eq: "MARY" } }) {
        edges { node { first_name } }
        totalCount
      }
    }`);
    data.hanaCustomerConnection.edges.forEach(e => expect(e.node.first_name).toBe('MARY'));
    expect(data.hanaCustomerConnection.totalCount).toBeGreaterThanOrEqual(1);
  });

  test('customerConnection with orderBy sorts correctly', async () => {
    const data = await client.request(gql`{
      hanaCustomerConnection(first: 5, orderBy: { first_name: "ASC" }) {
        edges { node { first_name } }
      }
    }`);
    const names = data.hanaCustomerConnection.edges.map(e => e.node.first_name);
...
describe('Mutations — CRUD', () => {
  ...
  test('create customer', async () => {
    const data = await client.request(gql`
      mutation {
        createHanaCustomer(input: { first_name: "E2E", last_name: "Test", email: "e2e@test.com", active: 1 }) {
          customer_id first_name last_name email
        }
      }
    `);
    expect(data.createHanaCustomer.first_name).toBe('E2E');
    expect(data.createHanaCustomer.customer_id).toBeTruthy();
    createdId = data.createHanaCustomer.customer_id;
  });

  test('update customer', async () => {
    if (!createdId) return;
    const data = await client.request(gql`
      mutation { updateHanaCustomer(where: { customer_id: { eq: ${createdId} } }, input: { first_name: "E2E_Updated" }) { customer_id first_name } }
    `);
    expect(data.updateHanaCustomer[0].first_name).toBe('E2E_Updated');
  });

  test('delete customer', async () => {
    if (!createdId) return;
    const data = await client.request(gql`
      mutation { deleteHanaCustomer(where: { customer_id: { eq: ${createdId} } }) { customer_id } }
    `);
    expect(data.deleteHanaCustomer[0]).not.toBeNull();
  });

  test('deleted customer is no longer queryable', async () => {
    if (!createdId) return;
    const data = await client.request(gql`{ hanaCustomer(where: { customer_id: { eq: ${createdId} } }) { customer_id } }`);
    expect(data.hanaCustomer).toEqual([]);
  });
...
describe('Relationships', () => {
  test('orders with nested customer (forward FK)', async () => {
    const data = await client.request(gql`{ hanaOrders(limit: 3) { order_id customer_id hanaCustomerId { customer_id first_name last_name } } }`);
    expect(data.hanaOrders.length).toBeGreaterThanOrEqual(1);
    expect(data.hanaOrders[0].hanaCustomerId).not.toBeNull();
  ...
  test('task with nested customer (forward FK)', async () => {
    const data = await client.request(gql`{ hanaTask(where: { customer_id: { isNotNull: true } }, limit: 3) { task_id title customer_id hanaCustomerId { customer_id first_name } } }`);
    expect(data.hanaTask.length).toBeGreaterThanOrEqual(1);
    expect(data.hanaTask[0].hanaCustomerId).not.toBeNull();
  });

  test('task with null customer_id has null customer', async () => {
    const data = await client.request(gql`{ hanaTask(where: { customer_id: { isNull: true } }, limit: 3) { task_id customer_id hanaCustomerId { customer_id } } }`);
    // If any tasks exist with null customer_id, the nested customer field should be null
    if (data.hanaTask.length > 0) {
      data.hanaTask.forEach(t => expect(t.hanaCustomerId ?? null).toBeNull());
    }
  });

  test('product_detail with nested product (forward FK)', async () => {
    const data = await client.request(gql`{ hanaProductDetail(limit: 3) { detail_id product_id hanaProductId { product_id name } } }`);
    expect(data.hanaProductDetail.length).toBeGreaterThanOrEqual(1);
    expect(data.hanaProductDetail[0].hanaProductId).not.toBeNull();
    expect(data.hanaProductDetail[0].hanaProductId.name.length).toBeGreaterThan(0);
  });

  test('customer with nested orders (reverse FK)', async () => {
    const data = await client.request(gql`{ hanaCustomer(where: { customer_id: { eq: 1 } }) { customer_id first_name hanaOrders { order_id total } } }`);
  ...
  test('product with nested product_details (reverse FK)', async () => {
    const data = await client.request(gql`{ hanaProduct(where: { product_id: { eq: 1 } }) { product_id name hanaProductDetail { detail_id } } }`);
    expect(data.hanaProduct[0].hanaProductDetail.length).toBeGreaterThanOrEqual(1);
  });
});
...
describe('Error handling', () => {
  test('filter with no matches returns empty array', async () => {
    const data = await client.request(gql`{ hanaCustomer(where: { first_name: { eq: "ZZZNOMATCH999" } }) { customer_id } }`);
    expect(data.hanaCustomer).toEqual([]);
  });
...
describe('ENUM columns', () => {
  test('task basic query with ENUM columns', async () => {
    const data = await client.request(gql`{ hanaTask { task_id title status priority } }`);
    expect(data.hanaTask.length).toBeGreaterThanOrEqual(5);
  });

  test('filter task by ENUM status', async () => {
    const data = await client.request(gql`{ hanaTask(where: { status: { eq: "done" } }) { task_id status } }`);
    expect(data.hanaTask.length).toBeGreaterThanOrEqual(1);
    data.hanaTask.forEach(r => expect(r.status).toBe('done'));
  });

  test('filter task by ENUM priority', async () => {
    const data = await client.request(gql`{ hanaTask(where: { priority: { eq: "high" } }) { task_id priority } }`);
    expect(data.hanaTask.length).toBeGreaterThanOrEqual(1);
  });

  test('task aggregate count', async () => {
    const data = await client.request(gql`{ hanaTaskAggregate { count } }`);
    expect(data.hanaTaskAggregate.count).toBeGreaterThanOrEqual(5);
  });

  test('create task with ENUM values', async () => {
    const data = await client.request(gql`
      mutation {
        createHanaTask(input: { title: "E2E Task", status: "in_progress", priority: "high" }) {
          task_id title status priority
        }
      }
    `);
    expect(data.createHanaTask.status).toBe('in_progress');
    expect(data.createHanaTask.priority).toBe('high');
  });
});
...
describe('JSON columns', () => {
  test('product_detail basic query', async () => {
    const data = await client.request(gql`{ hanaProductDetail { detail_id product_id attributes metadata tags } }`);
    expect(data.hanaProductDetail.length).toBeGreaterThanOrEqual(3);
  });

  test('JSON columns are non-null', async () => {
    const data = await client.request(gql`{ hanaProductDetail(limit: 1) { attributes metadata tags } }`);
    expect(data.hanaProductDetail[0].attributes).not.toBeNull();
  });

  test('filter product_detail by product_id', async () => {
    const data = await client.request(gql`{ hanaProductDetail(where: { product_id: { eq: 1 } }) { detail_id product_id } }`);
    expect(data.hanaProductDetail.length).toBeGreaterThanOrEqual(1);
  });
  ...
  test('create product_detail with JSON', async () => {
    ...
    `);
    expect(data.createHanaProductDetail.detail_id).toBeTruthy();
  });
});
...
describe('Views (read-only)', () => {
  test('active_customers view query', async () => {
    ...
    expect(data.hanaActiveCustomers.length).toBeGreaterThanOrEqual(10);
  });

  test('active_customers excludes inactive rows', async () => {
    const data = await client.request(gql`{ hanaCustomer(where: { active: { eq: 0 } }) { customer_id active } }`);
    expect(data.hanaCustomer.length).toBeGreaterThanOrEqual(2);
  });

  test('orders_summary view query', async () => {
    const data = await client.request(gql`{ hanaOrdersSummary { customer_id first_name last_name order_count total_spent } }`);
    // LEFT JOIN — customers with no orders have order_count=0, total_spent=null
    expect(data.hanaOrdersSummary.length).toBeGreaterThanOrEqual(5);
    const withOrders = data.hanaOrdersSummary.filter(r => r.order_count > 0);
  ...
  test('high_value_orders view — all totals > 50', async () => {
    const data = await client.request(gql`{ hanaHighValueOrders { order_id total status first_name last_name } }`);
    expect(data.hanaHighValueOrders.length).toBeGreaterThanOrEqual(1);
    data.hanaHighValueOrders.forEach(r => expect(r.total).toBeGreaterThan(50));
  });
  ...
  test('view ordering', async () => {
    const data = await client.request(gql`{ hanaActiveCustomers(orderBy: { customer_id: "ASC" }, limit: 3) { customer_id } }`);
    expect(data.hanaActiveCustomers[0].customer_id).toBeLessThan(data.hanaActiveCustomers[1].customer_id);
  });

  test('view aggregate count', async () => {
    const data = await client.request(gql`{ hanaActiveCustomersAggregate { count } }`);
    expect(data.hanaActiveCustomersAggregate.count).toBeGreaterThanOrEqual(10);
  });
