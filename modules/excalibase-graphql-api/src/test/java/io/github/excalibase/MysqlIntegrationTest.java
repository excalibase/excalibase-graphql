...
class MysqlIntegrationTest {
    ...
    static void configureProperties(DynamicPropertyRegistry registry) {
        ...
        registry.add("app.schema", mysql::getDatabaseName);
    ...
    void listCustomers() throws Exception {
        mockMvc.perform(post("/graphql")
                        ...
                        .content(graphql("{ customer(orderBy: { customer_id: ASC }, limit: 5) { customer_id first_name last_name } }")))
    ...
    void listWithWhereFilter() throws Exception {
        mockMvc.perform(post("/graphql")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(graphql("{ customer(where: { active: { eq: true } }) { customer_id } }")))
                .andExpect(status().isOk())
    ...
    void forwardFkRelationship() throws Exception {
        mockMvc.perform(post("/graphql")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(graphql("{ orders(limit: 1, orderBy: { order_id: ASC }) { order_id total_amount customer { first_name } } }")))
                .andExpect(status().isOk())
    ...
    void reverseFkRelationship() throws Exception {
        mockMvc.perform(post("/graphql")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(graphql("{ customer(where: { customer_id: { eq: 1 } }) { first_name orders { order_id } } }")))
                .andExpect(status().isOk())
    ...
    void connectionBasic() throws Exception {
        mockMvc.perform(post("/graphql")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(graphql("{ customerConnection(first: 2) { edges { cursor node { customer_id first_name } } pageInfo { hasNextPage } totalCount } }")))
                .andExpect(status().isOk())
    ...
    void createCustomer() throws Exception {
        mockMvc.perform(post("/graphql")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(graphql("mutation { createCustomer(input: { first_name: \"Test\", last_name: \"MySQL\" }) { customer_id first_name } }")))
                .andExpect(status().isOk())
    ...
    void updateCustomer() throws Exception {
        mockMvc.perform(post("/graphql")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(graphql("mutation { updateCustomer(input: { customer_id: 1, email: \"updated@mysql.com\" }) { customer_id email } }")))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.updateCustomer.email").value("updated@mysql.com"));
    }
    ...
    void deleteCustomer() throws Exception {
        ...
        int createdId = com.jayway.jsonpath.JsonPath.read(
                createResult.getResponse().getContentAsString(), "$.data.createCustomer.customer_id");

        mockMvc.perform(post("/graphql")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(graphql("mutation { deleteCustomer(input: { customer_id: " + createdId + " }) { customer_id first_name } }")))
                .andExpect(status().isOk())
    ...
    void whereNeqFilter() throws Exception {
        mockMvc.perform(post("/graphql")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(graphql("{ customer(where: { first_name: { neq: \"Alice\" } }, limit: 10) { first_name } }")))
                .andExpect(status().isOk())
    ...
    void whereGteFilter() throws Exception {
        mockMvc.perform(post("/graphql")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(graphql("{ orders(where: { total_amount: { gte: 250 } }) { order_id } }")))
                .andExpect(status().isOk())
    ...
    void whereLtFilter() throws Exception {
        mockMvc.perform(post("/graphql")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(graphql("{ orders(where: { total_amount: { lt: 100 } }) { order_id } }")))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.orders", hasSize(2)));
    }
    ...
    void orderByDesc() throws Exception {
        ...
        mockMvc.perform(post("/graphql")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(graphql("{ customer(orderBy: { first_name: DESC }, limit: 1, where: { customer_id: { lte: 5 } }) { first_name } }")))
                .andExpect(status().isOk())
    ...
    void multipleRootFields() throws Exception {
        mockMvc.perform(post("/graphql")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(graphql("{ customer(limit: 1, orderBy: { customer_id: ASC }) { first_name } orders(limit: 1, orderBy: { order_id: ASC }) { order_id } }")))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.customer[0].first_name").value("Alice"))
    ...
    void whereIsNotNullFilter() throws Exception {
        ...
        mockMvc.perform(post("/graphql")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(graphql("{ customer(where: { email: { isNotNull: true } }) { customer_id email } }")))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.customer", hasSize(5)));
    }
    ...
    void orderByDescString() throws Exception {
        mockMvc.perform(post("/graphql")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(graphql("{ customer(orderBy: { customer_id: \"DESC\" }, limit: 3, where: { customer_id: { lte: 5 } }) { customer_id } }")))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.customer[0].customer_id").value(5))
                .andExpect(jsonPath("$.data.customer[1].customer_id").value(4));
    }
    ...
    void queryView() throws Exception {
        mockMvc.perform(post("/graphql")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(graphql("{ activeCustomers(limit: 10) { customer_id first_name } }")))
                .andExpect(status().isOk())
    ...
    void viewHasNoMutationFields() throws Exception {
        mockMvc.perform(post("/graphql")
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.__type.fields[?(@.name == 'createActiveCustomers')]").doesNotExist());
    }
    ...
    void queryTaskWithEnumColumns() throws Exception {
        mockMvc.perform(post("/graphql")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(graphql("{ task(orderBy: { task_id: ASC }, limit: 3) { task_id title status priority } }")))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.task", hasSize(3)))
                .andExpect(jsonPath("$.data.task[0].title").value("Fix bug"))
                .andExpect(jsonPath("$.data.task[0].status").value("done"))
    ...
    void filterTaskByEnumStatus() throws Exception {
        mockMvc.perform(post("/graphql")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(graphql("{ task(where: { status: { eq: \"done\" } }) { task_id title } }")))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.task", hasSize(1)));
    }
    ...
    void taskForwardFkToCustomer() throws Exception {
        mockMvc.perform(post("/graphql")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(graphql("{ task(orderBy: { task_id: ASC }, limit: 1) { title customer { first_name } } }")))
                .andExpect(status().isOk())
